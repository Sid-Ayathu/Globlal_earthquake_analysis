...
for i in range(1, len(df)):
    ...
    if current_date == previous_date:
        ...
        # if 0.5<current_magnitude <= 2.5 and 0.5<previous_magnitude <= 2.5 and 60 < time_diff_in_seconds < 600 and distance_km > 10 and distance_km < 100:
        #     current_group.append(df.iloc[i])
        if 3.0 < higher_magnitude <= 4.0 and time_diff_minutes<100  and distance_km > 3 :
            # Group if the higher magnitude is between 2.5 and 4.0, and time difference is between 2 and 20 minutes
            current_group.append(df.iloc[i])
        elif 4.0<higher_magnitude <= 6.0 and time_diff_minutes < 120 and distance_km > 5 :
...
if groups:
    ...
    grouped_df.to_csv('mainshock_grouped_chunks/mainshock_grouped_chunk6_new.csv', index=False)
    print('Grouped earthquakes have been saved to "mainshock_grouped_chunk6_new.csv".')
else:
